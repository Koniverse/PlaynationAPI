...
export async function checkQuest() {
  ...
  const dataQuest = await ZealyService.instance.addAction<ResponseZealy>(ZealyActionRoutes.ReviewQuest, 'v2', 'GET', {
  ...
  if (dataQuest) {
    ...
    if (claimFailIds.length > 0) {
      ...
      await ZealyService.instance.addAction(ZealyActionRoutes.ClaimedQuestsReview, 'v1', 'POST', dataReview);
    }
